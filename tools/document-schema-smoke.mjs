...
const safeLocal3d = fs.readFileSync('src/document/safe-local-3d.js', 'utf8')
...
const inputController = fs.readFileSync('src/controllers/input-controller.js', 'utf8')
const renderController = fs.readFileSync('src/controllers/render-controller.js', 'utf8')
...
check(safeLocal3d.includes("import('./local-3d.js')"), 'local 3D runtime is loaded on demand')
check(!safeLocal3d.includes("import { Local3DManager } from './local-3d.js'"), 'local 3D has no eager manager import')
...
check(renderController.includes('app.documentRuntime?.paint?.'), 'render controller delegates document painting explicitly')
check(renderController.includes('app.documentRuntime?.frame?.(ms)'), 'render controller schedules document runtime from its RAF')
...
check(inputController.includes("articleReaderScroll('line-down')"), 'ArrowDown routes into document reader')
check(inputController.includes("articleReaderScroll('line-up')"), 'ArrowUp routes into document reader')
check(renderController.includes('if (documentItem) {'), 'document details bypass hidden terminal block rendering')
check(renderController.includes('DOCUMENT VIEW ACTIVE'), 'document terminal fallback stays media-free')
