...
const penwModel = JSON.parse(fs.readFileSync('content/projects/penw/assets/arcade-cabinet.model.json', 'utf8'))
...
check(plugin.includes("'.json': 'application/json'"), 'local model manifest inlining supported')
...
check(penw.includes('::model3d{src=assets/arcade-cabinet.model.json'), 'PENW uses local Three.js model block')
check(!penw.includes('provider=sketchfab'), 'PENW cabinet no longer depends on Sketchfab')
check(penwModel.format === 'jg1500-model-1', 'PENW local model manifest has supported format')
check(penwModel.parts?.length === 29, 'PENW local model preserves 29 source parts')
