...
check(reader.includes("video.preload = 'none'"), 'local video defaults to no eager preload')
...
check(authoring.includes('lazy'), 'authoring guide documents media loading policy')
check(design.includes('no Leak-specific runtime code'), 'architecture records anti-overfit rule')
check(design.includes('preload="none"'), 'architecture records local-video loading policy')
