...
export class DocumentProgressOverlay {
  ...
  paint(rasteriser) {
    ...
    const hasChapterProgress = chapters.length >= this.minChapters
    const scroll = rasteriser.scroll || 0
    const showReadingHint = Boolean(rasteriser.fullscreen && rasteriser.maxScroll > 0 && scroll <= 2)
    if (!hasChapterProgress && !showReadingHint) return false
    ...
    const active = hasChapterProgress ? this._activeIndex(chapters, scroll) : 0
    ...
    if (showReadingHint) {
      g.font = '600 7px ui-monospace, "SFMono-Regular", Consolas, monospace'
      g.textAlign = 'left'
      g.fillStyle = 'rgba(107,243,154,.58)'
      g.fillText('SCROLL TO READ', Math.max(18, rasteriser.columnX || 18), height - 17)
    }
