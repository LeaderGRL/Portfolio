...
export function fullscreenLayout(width, height, dpr = 1, controlsHeight = 0) {
  width = Math.max(1, width)
  height = Math.max(1, height)
  const density = Math.min(Math.max(1, dpr), 2, 4096 / Math.max(width, height), Math.sqrt(8388608 / (width * height)))
